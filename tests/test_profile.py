...
class TestProfileView:
    ...
    def test_profile_view_get(self, client, post_with_group):
        try:
            ...
            assert False, f'''Страница `/profile/<username>/`
        ...
        assert profile_context is not None, (
            'Проверьте, что передали '
            + 'автора в контекст страницы `/profile/<username>/`'
        )

        page_context = get_field_from_context(response.context, Page)
        assert page_context is not None, (
            'Проверьте, что передали статьи автора в '
            + 'контекст страницы `/profile/<username>/` типа `Page`'
        )
        assert len(page_context.object_list) == 1, (
            'Проверьте, что правильные статьи '
            + 'автора в контекст страницы `/profile/<username>/`'
        )
        ...
        assert page_context is not None, (
            'Проверьте, что передали статьи '
            + 'автора в контекст страницы `/profile/<username>/` типа `Page`'
        )
