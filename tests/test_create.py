...
class TestCreateView:
    ...
    def test_create_view_get(self, user_client):
        try:
            ...
            assert False, f'''Страница `/create/`
        ...
        assert 'form' in response.context, (
            ...
            + ' `form` в контекст страницы `/create/`'
        )
        ...
        assert 'group' in response.context['form'].fields, (
            ...
            + 'на странице `/create/` есть поле `group`'
        )
        assert type(
            response.context['form'].fields['group']
        ) == forms.models.ModelChoiceField, (
            'Проверьте, что в форме `form` на'
            + 'странице `/create/` поле `group` типа `ModelChoiceField`'
        )
        assert not response.context['form'].fields['group'].required, (
            'Проверьте, что в форме `form`'
            + 'на странице `/create/` поле `group` не обязательно'
        )

        assert 'text' in response.context['form'].fields, (
            'Проверьте, что в форме `form`'
            + 'на странице `/create/` есть поле `text`'
        )
        assert type(
            response.context['form'].fields['text']
        ) == forms.fields.CharField, (
            'Проверьте, что в форме `form`'
            + 'на странице `/create/` поле `text` типа `CharField`'
        )
        assert response.context['form'].fields['text'].required, (
            'Проверьте, что в форме `form`'
            + 'на странице `/create/` поле `text` обязательно'
        )
    ...
    def test_create_view_post(self, user_client, user, group):
        ...
        try:
            ...
            assert False, f'''Страница `/create`
            работает неправильно. Ошибка: `{e}`'''
        url = '/create/' if response.status_code in (
            301, 302
        ) else '/create'
        ...
        assert response.status_code in (301, 302), (
            ...
            f'перенаправляете на страницу'
            f'профиля автора `/profile/{user.username}`'
        )
        post = Post.objects.filter(
            author=user, text=text, group=group
        ).first()
        ...
        assert response.url == f'/profile/{user.username}/', (
            f'Проверьте, что перенаправляете на'
            f'страницу профиля автора `/profile/{user.username}`'
        )
        ...
        assert post is not None, (
            'Проверьте, что вы сохранили новый пост'
            + 'при отправке формы на странице `/create/`'
        )
        assert response.url == f'/profile/{user.username}/', (
            f'Проверьте, что перенаправляете на'
            f'страницу профиля автора `/profile/{user.username}`'
        ...
        assert response.status_code == 200, (
            'Проверьте, что на странице `/create/`'
